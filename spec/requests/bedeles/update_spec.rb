...
RSpec.describe 'Bedeles', type: :request do
  before do
    Bedel.create!(id: 'tt1', turno: 'mañana', nombre: 'test_nombre', apellido: 'test_apellido', password: '12&A45678')
  end
  ...
  scenario 'Send put request to update bedel\'s password' do
    put 'http://localhost:3000/bedeles/tt1', params: { password: '12&A45678UP' }

    expect(response).to have_http_status(:success)
    bedel = Bedel.find('tt1')
    expect(bedel.authenticate('12&A45678UP')).to eq(bedel)
    expect(bedel.authenticate('12&A45678')).to eq(false)
  end
end
